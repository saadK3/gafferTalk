...
class SquadActionDecisionService:
    ...
    def research(
        ...
        *,
        ...
        best_transfer = actions[0] if actions else None
        recommended = (
            best_transfer
            if best_transfer is not None
            and best_transfer.policy_adjusted_gain
            >= (policy.hit_threshold if best_transfer.points_hit else policy.roll_threshold)
            else roll
        )
        compared = tuple(
            [recommended, roll, *[item for item in actions[:3] if item != recommended]]
        )
        compared = tuple(dict.fromkeys(compared))
        evidence_ids = self._evidence_ids(concerns, best_transfer)
        evidence = tuple(
        ...
        confidence = self._confidence(
            ...
            separation=(best_transfer.policy_adjusted_gain if best_transfer else 0),
        )
        hit_analysis = self._hit_analysis(best_transfer, policy)
        priority_explanation = concerns[0].explanation
        planning_impact = self._planning_impact(recommended, state)
        grounded_reasons = (
            GroundedReason(id="recommended_action", text=recommended.explanation),
            GroundedReason(id="leading_priority", text=priority_explanation),
            GroundedReason(id="roll_comparison", text=self._roll_comparison(recommended, policy)),
        ...
        return SquadActionReport(
            ...
            evidence=evidence,
            assumptions=(
                "All 15 selling prices, the bank and free-transfer count are manager-confirmed.",
    ...
    def preview_evidence_ids(
        ...
        *,
        ...
        return self._evidence_ids(concerns, actions[0] if actions else None)
    ...
    def _validate_state(
        ...
        if set(state.selling_prices) != squad_ids:
            raise ValueError("confirm a selling price for every player in the squad")
        ...
        for pick in snapshot.picks:
            ...
            for incoming in catalogue.players.values():
                ...
                action = SquadActionCandidate(
                    ...
                    explanation=(
                        f"Sell {player.web_name} for {incoming.web_name}; the route improves the "
                        f"evidence ranking by {raw_gain:.1f}, leaves "
                    ...
                    if priority > 0:
                        concern_rows.append(
                            (
                                ...
                                player,
                                ConcernKind.UPGRADE,
                                f"{player.web_name} has a legal upgrade worth "
